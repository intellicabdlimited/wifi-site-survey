...
# ============================================================
# 8) RUN: upload any number of DOCX, then extract
# ============================================================
# print("Upload one or more DOCX files.")
# uploaded = files.upload()

# docx_paths = [f"/content/{name}" for name in uploaded.keys() if name.lower().endswith(".docx")]
# if not docx_paths:
...
def main():
    if not os.path.isdir(INPUT_ROOT):
        raise ValueError(f"Input folder not found: {INPUT_ROOT} (make sure extractor created /content/out)")

    ensure_dir(RESULT_ROOT)

    heatmaps = scan_heatmaps(INPUT_ROOT)
    metric_keys_menu = build_inventory(heatmaps)

    print("\n================ INVENTORY ================")
    print(f"Total heatmaps: {len(heatmaps)}")

    if not metric_keys_menu:
        print("No usable metrics found. Check filenames and metric parsing.")
        return

    metric_menu = [f"{k}  ({METRIC_CFG_BY_KEY[k]['name']})" for k in metric_keys_menu]
    metric_idx = pick("\nSelect metric:", metric_menu)
    selected_metric_key = metric_keys_menu[metric_idx]
    metric_cfg = METRIC_CFG_BY_KEY[selected_metric_key]

    loc_idx = pick("\nSelect location:", [x[0] for x in LOCATIONS])
    LOCATION = LOCATIONS[loc_idx][0]

    BAND = None
    if bool(metric_cfg.get("has_band", True)):
        band_idx = pick("\nSelect band:", [x[0] for x in BANDS])
        BAND = BANDS[band_idx][0]

    reverse_lr = False  # LOW->HIGH left-to-right (unchanged)

    picked = []
    for p in heatmaps:
        fn = os.path.basename(p)

        if not matches_location(fn, LOCATION):
            continue

        k = metric_key_from_filename(fn)
        if k != selected_metric_key:
            continue

        if BAND is not None and not matches_band(fn, BAND):
            continue

        picked.append(p)

    picked = sorted(picked)
    if DEBUG_MAX_HEATMAPS is not None:
        picked = picked[:int(DEBUG_MAX_HEATMAPS)]

    metric_slug = slug(selected_metric_key)
    band_slug = slug(BAND) if BAND else "no_band"
    run_dir = unique_dir(os.path.join(RESULT_ROOT, f"{metric_slug}__{slug(LOCATION)}__{band_slug}"))
    ensure_dir(run_dir)

    print("\n================ SELECTION ================")
    print("Metric   :", metric_cfg["name"])
    print("Location :", LOCATION)
    print("Band     :", BAND if BAND else "(not applicable)")
    print("Scale    : internal auto (numeric vs categorical by metric + legend)")
    print("Heatmaps selected:", len(picked))
    ...
    for hm_path in picked:
        hm_name = os.path.basename(hm_path)
        hm_stem = os.path.splitext(hm_name)[0]
        tag = f"{slug(hm_stem)}__"

        debug_dir = None
        if DEBUG:
            debug_dir = ensure_dir(os.path.join(run_dir, "_debug", slug(hm_stem)))

        try:
            scale_path = find_matching_scale(hm_path)

            if DEBUG and debug_dir:
                with open(os.path.join(debug_dir, "pairing.json"), "w") as f:
                    json.dump(dict(heatmap=hm_path, scale=scale_path), f, indent=2)

            scale_rgb = read_image_rgb(scale_path)
            hm_rgb = read_image_rgb(hm_path)

            if selected_metric_key == "snr":
                df_hex, debug = extract_hexagons(hm_rgb, roi_sat_thresh=6, roi_val_thresh=254)
            else:
                df_hex, debug = extract_hexagons(hm_rgb)

            p_edges = os.path.join(run_dir, f"{hm_stem}_edges.png")
            p_roi   = os.path.join(run_dir, f"{hm_stem}_roi.png")
            p_int   = os.path.join(run_dir, f"{hm_stem}_interior.png")
            save_gray(debug["edges"], p_edges)
            save_gray(debug["roi"], p_roi)
            save_gray(debug["interior"], p_int)
            out_files.extend([p_edges, p_roi, p_int])

            df_hex = assign_row_major_ids(df_hex)

            if metric_cfg.get("scale_kind") == "categorical":
                cat_model = build_categorical_scale_model(scale_rgb, reverse_lr=reverse_lr, debug_dir=debug_dir, tag=tag)
                df_vals = map_hex_to_categorical(df_hex, cat_model, debug_dir=debug_dir, tag=tag)

                tmp = df_vals.copy()
                tmp["value_mid"] = tmp["scale_idx"]
                p_annot = os.path.join(run_dir, f"{hm_stem}_annotated.png")
                annotate_hexes_id_value_clear(hm_rgb, tmp, p_annot, value_col="value_mid")
                out_files.append(p_annot)

                p_hist = os.path.join(run_dir, f"{hm_stem}_histogram.png")
                save_histogram_categorical(df_vals, cat_model.steps.reset_index(drop=True), p_hist, metric_cfg["axis_label"])
                out_files.append(p_hist)

                p_csv = os.path.join(run_dir, f"{hm_stem}_output.csv")
                write_clean_output_csv(df_vals, p_csv, is_categorical=True)
                out_files.append(p_csv)
                ...
                meta_rows.append({
                    ...
                    "device": device_from_filename(hm_path),
                    "band": detect_band_from_name(hm_name),
                    "n_bins": int(len(cat_model.steps)),
                    "mode": "categorical",
                    "csv": p_csv
                })

                log(f"OK: {hm_name} | categorical | bins: {len(cat_model.steps)}")

            else:
                device = device_from_filename(hm_path)
                mn_user, mx_user = ocr_minmax_from_scale_path(scale_path, metric_cfg, debug_dir=debug_dir, tag=tag)
                ...
                df_vals = map_hex_to_numeric(df_hex, num_model, debug_dir=debug_dir, tag=tag)

                p_annot = os.path.join(run_dir, f"{hm_stem}_annotated.png")
                annotate_hexes_id_value_clear(hm_rgb, df_vals, p_annot, value_col="value_mid")
                out_files.append(p_annot)

                p_hist = os.path.join(run_dir, f"{hm_stem}_histogram.png")
                save_histogram_colored(df_vals, num_model.steps.reset_index(drop=True), p_hist, metric_cfg["axis_label"])
                out_files.append(p_hist)

                p_csv = os.path.join(run_dir, f"{hm_stem}_output.csv")
                write_clean_output_csv(df_vals, p_csv, is_categorical=False)
                out_files.append(p_csv)
                ...
                log(f"OK: {hm_name} | numeric/{num_model.kind} | bins: {len(num_model.steps)}")

        except Exception as e:
            failed.append((hm_path, str(e)))
            log(f"FAIL: {hm_name} => {e}")

    meta_df = pd.DataFrame(meta_rows)
    index_csv = os.path.join(run_dir, "_index.csv")
    meta_df.to_csv(index_csv, index=False)
    out_files.append(index_csv)

    # if (not meta_df.empty) and ("scale_kind" in meta_df.columns) and (meta_df["scale_kind"] == "numeric").all():
    #     metric_cfg2 = METRIC_CFG_BY_KEY.get(selected_metric_key, None)
    #     if metric_cfg2 is not None:
    #         rows = []
    ...
    #             df2 = pd.DataFrame({"value": v}).dropna()
    #             df2["device"] = r["device"]
    #             df2["band"] = r["band"]
    #             rows.append(df2)
    ...
    #         if BAND is not None and not all_df.empty and all_df["device"].nunique() >= 2:
    #             df_b = all_df[all_df["band"] == BAND].copy()
    #             if not df_b.empty:
    #                 comp_dir = ensure_dir(os.path.join(run_dir, "_comparison"))
    #                 out_b = ensure_dir(os.path.join(comp_dir, slug(BAND)))
    #                 out_files.extend(comparison_outputs(df_b, out_b, metric_cfg2, BAND))

    if failed:
        fail_txt = os.path.join(run_dir, "failed_items.txt")
        with open(fail_txt, "w") as f:
            for p, err in failed:
                f.write(f"{p}\n{err}\n\n")
        out_files.append(fail_txt)

    zip_name = f"{slug(selected_metric_key)}__{slug(LOCATION)}__{slug(BAND) if BAND else 'no_band'}.zip"
    zip_path = os.path.join("/content", zip_name)

    with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_STORED) as z:
        for root, _, files_list in os.walk(run_dir):
            if "_debug" in Path(root).parts:
                continue
