...
INPUT_ROOT = str(BASE_DIR / "out")
RESULT_ROOT = str(BASE_DIR / "ss_results")
...
# ============================================================
# METRIC / LOCATION CONFIG
# ============================================================
DBM_EDGES_FALLBACK = [
    -np.inf, -90, -85, -80, -75, -70, -67, -65,
    -60, -55, -50, -45, -40, -35, -30, np.inf
]
...
METRICS = [
    ...
    dict(
        ...
        scale_kind="numeric",
        has_band=True,
        preset_minmax=(5.0, 40.0),   # fallback only if OCR fails
    ),
    dict(
        key="noise",
        name="Noise",
...
def process_many_docx_local(docx_paths, out_root="out", download_per_docx_zip=True, also_make_master_zip=False):
    ...
    for docx_path in docx_paths:
        ...
        zip_base = os.path.join(out_root, f"{device}_{stem}_extracted")
        zip_file = shutil.make_archive(zip_base, "zip", out_dir)
        print("ZIP created:", zip_file)

        zip_files.append(zip_file)

        if download_per_docx_zip:
            print(f"ZIP saved to: {zip_file}")

    if also_make_master_zip:
        master_base = os.path.join(out_root, "ALL_EXTRACTED")
        master_zip = shutil.make_archive(master_base, "zip", out_root)
        print("\nMASTER ZIP created:", master_zip)
        zip_files.append(master_zip)
        if download_per_docx_zip:
            print(f"MASTER ZIP saved to: {master_zip}")
...
def ocr_minmax_from_scale_path(scale_path: str, metric_cfg: dict, debug_dir: str | None = None, tag: str = ""):
    ...
    if mn is None or mx is None:
        if preset_minmax is not None:
            ...
            dlog(
                f"OCR fallback used for {os.path.basename(scale_path)} "
                f"-> mn={mn}, mx={mx}"
            )
        else:
            raise ValueError(
...
def detect_bar_and_colors(scale_rgb: np.ndarray, strip_y_frac=(0.75, 0.98), major_min_frac=0.008,
                          debug_dir: str | None = None, tag: str = ""):
...
def build_numeric_scale_model(scale_rgb: np.ndarray, metric_cfg: dict, reverse_lr: bool, mn: float, mx: float,
                              debug_dir: str | None = None, tag: str = "") -> NumericScaleModel:
                ...
                with open(os.path.join(debug_dir, f"{tag}numeric_discrete_meta.json"), "w") as f:
                    json.dump(
                        dict(
                            ...
                            edge_mode=edge_mode,
                            edges=[
                                float(x) if np.isfinite(x)
                                else (float("inf") if np.isposinf(x) else -float("inf"))
                                for x in edges
                            ],
                        ),
                        f,
...
# ============================================================
def extract_hexagons(heatmap_rgb: np.ndarray, canny1=30, canny2=100, edge_dilate=1,
                     roi_sat_thresh=15, roi_val_thresh=250,
                     area_min_factor=0.75, area_max_factor=1.35):
...
def map_hex_to_numeric(df_hex: pd.DataFrame, model: NumericScaleModel,
                       debug_dir: str | None = None, tag: str = "") -> pd.DataFrame:
    ...
    if model.kind == "discrete":
        ...
        steps = model.steps.reset_index(drop=True)
        edges = np.array(
            [float(x) for x in steps["value_min"].tolist()] + [float(steps["value_max"].iloc[-1])],
            dtype=float,
        )
...
def map_hex_to_categorical(df_hex: pd.DataFrame, model: CategoricalScaleModel,
                           debug_dir: str | None = None, tag: str = "") -> pd.DataFrame:
    rgb = df_hex[["r", "g", "b"]].to_numpy().astype(np.uint8)
    lab = rgb_to_lab(rgb)
...
# OUTPUTS / PLOTS
# ============================================================
def annotate_hexes_id_value_clear(img_rgb: np.ndarray, df: pd.DataFrame, out_file: str,
                                  value_col="value_mid", font_scale=0.32, thickness=1,
                                  pad_x=3, pad_y=2, box_alpha=0.65, border_px=1):
...
def main():
    if not os.path.isdir(INPUT_ROOT):
        raise ValueError(f"Input folder not found: {INPUT_ROOT} (make sure extractor created {INPUT_ROOT})")
    ...
    print("ZIP:", zip_path)
    print(f"Final ZIP saved to: {zip_path}")


if __name__ == "__main__":
    DOCX_INPUT_DIR = BASE_DIR / "docx_input"
    DOCX_INPUT_DIR.mkdir(exist_ok=True)

    docx_paths = [str(p) for p in DOCX_INPUT_DIR.glob("*.docx")]
    if not docx_paths:
        raise ValueError(f"No .docx files found in: {DOCX_INPUT_DIR}")

    process_many_docx_local(
        docx_paths,
        out_root=str(BASE_DIR / "out"),
