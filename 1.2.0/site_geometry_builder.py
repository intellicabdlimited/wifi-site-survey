...
def pick_points_local(image_path: str, n_points: int = 1, title: str = "Click points") -> np.ndarray:
    """Open a desktop popup for point picking without relying on OpenCV HighGUI/Qt.

    This avoids the Qt font/Wayland issues that can appear with cv2.imshow on Linux
    while keeping the same user-facing popup workflow. If Tk is unavailable, the
    function falls back to the older OpenCV window path.
    """
    try:
        ...
        root = tk.Tk()
        root.title(f"{title} :: {os.path.basename(image_path)}")
        root.configure(bg="#1e1e1e")

        screen_w = max(root.winfo_screenwidth() - 160, 900)
        screen_h = max(root.winfo_screenheight() - 220, 700)
        scale = min(screen_w / img.width, screen_h / img.height, 1.0)
        disp_w = max(1, int(round(img.width * scale)))
        disp_h = max(1, int(round(img.height * scale)))
        disp_img = img.resize((disp_w, disp_h), Image.Resampling.LANCZOS) if scale != 1.0 else img.copy()

        top = tk.Frame(root, bg="#1e1e1e")
        top.pack(fill="x", padx=10, pady=(10, 6))
        status_var = tk.StringVar(value=f"Click {n_points} point(s). Press Backspace to undo. Esc cancels.")
        tk.Label(
            top,
        ...
        canvas = tk.Canvas(root, width=disp_w, height=disp_h, highlightthickness=0, cursor="crosshair")
        ...
        def redraw() -> None:
            ...
            for idx, (x, y) in enumerate(pts, start=1):
                ...
                sy = y * scale
                r = 7
                draw.ellipse((sx - r, sy - r, sx + r, sy + r), fill=(255, 0, 0), outline=(255, 255, 255), width=2)
                draw.text((sx + 10, sy - 14), str(idx), fill=(255, 0, 0))
            photo2 = ImageTk.PhotoImage(overlay)
            canvas.image = photo2
            canvas.itemconfigure(image_item, image=photo2)
            status_var.set(f"Selected {len(pts)}/{n_points} point(s). Backspace undoes. Esc cancels.")

        def finish() -> None:
            if len(pts) < int(n_points):
                messagebox.showinfo("Need more points", f"Please click {n_points} point(s) before continuing.")
                return
            done["value"] = True
            root.quit()
        ...
        def on_click(event) -> None:
            if len(pts) >= int(n_points):
                return
            x = min(max(event.x / scale, 0.0), float(img.width - 1))
            y = min(max(event.y / scale, 0.0), float(img.height - 1))
            pts.append((float(x), float(y)))
            redraw()
            if len(pts) >= int(n_points):
                finish()

        def on_backspace(event=None) -> None:
            if pts:
                pts.pop()
                redraw()
        ...
        tk.Button(btns, text="Undo", command=on_backspace).pack(side="left")
        tk.Button(btns, text="Cancel", command=on_escape).pack(side="left", padx=(8, 0))
        tk.Button(btns, text="Done", command=finish).pack(side="right")
        ...
        if cancelled["value"]:
            raise KeyboardInterrupt("Point picking cancelled.")
        if len(pts) < int(n_points):
            raise KeyboardInterrupt("Point picking cancelled before enough points were selected.")
        return np.array(pts[: int(n_points)], dtype=float)
        ...
        try:
            ...
            raise RuntimeError(
                "Interactive site geometry creation could not open a popup window. "
                "On Linux, install python3-tk and use a desktop session, or upload an existing site_geometry.json instead."
            ) from cv_exc
        ...
        while True:
            ...
            if key == 27:
                cv2.destroyWindow(win)
                raise KeyboardInterrupt("Point picking cancelled (ESC).") from tk_exc
            if len(pts) >= int(n_points):
                cv2.destroyWindow(win)
                break
