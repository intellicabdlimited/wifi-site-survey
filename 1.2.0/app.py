...
from site_geometry_builder import create_site_geometry, find_master_esx

from docx_extractor_v2 import process_many_docx_local

try:
    from ocr_csv_generator_v2 import run_ocr_generate_csv
    OCR_RUNNER_NAME = "ocr_csv_generator_v2"
except ImportError:
    from ocr_csv_generator import run_ocr_generate_csv
    OCR_RUNNER_NAME = "ocr_csv_generator"
...
def default_site_geometry_mode(saved_path: Optional[Path]) -> str:
    if saved_path and saved_path.exists():
        return "Use saved JSON"
    return "Upload existing JSON"
...
def resolve_site_geometry(
    *,
    ...
    target_path: Path,
    uploaded_file=None,
    master_esx_path: Optional[Path] = None,
    router_esx_paths: Optional[List[Path]] = None,
    all_esx_paths: Optional[List[Path]] = None,
) -> Path:
    target_path = Path(target_path)
    target_path.parent.mkdir(parents=True, exist_ok=True)

    if mode == "Upload existing JSON":
        if uploaded_file is None:
            raise ValueError("Upload a site_geometry.json file or choose another site geometry option.")
        write_uploaded_file(uploaded_file, target_path)
        return target_path

    if mode == "Use saved JSON":
        if not target_path.exists():
            raise FileNotFoundError(f"No saved site_geometry.json was found at {target_path}.")
        return target_path

    if mode != "Create from floor-plan popup":
        raise ValueError(f"Unknown site geometry mode: {mode}")

    esx_pool = [Path(p) for p in (all_esx_paths or [])]
    if master_esx_path is None:
        if not esx_pool:
            raise ValueError("A master ESX is required to create site geometry.")
        master_esx_path = find_master_esx(esx_pool)
    master_esx_path = Path(master_esx_path)

    router_paths = [Path(p) for p in (router_esx_paths or [])]
    if not router_paths and esx_pool:
        router_paths = [
            p for p in esx_pool
            if os.path.abspath(str(p)) != os.path.abspath(str(master_esx_path))
        ]

    return create_site_geometry(
        master_esx=master_esx_path,
        router_esx_paths=router_paths,
...
<div class="subtle">Reads from <code>runs/&lt;router_name&gt;/extracted</code> and writes grouped CSVs to <code>runs/&lt;router_name&gt;/csv_outputs/&lt;metric&gt;</code>. Uses the manifest-aware OCR wrapper when available.</div>
""",
        unsafe_allow_html=True,
    )
    ...
    if not extracted_ready:
        ...
        max_heatmaps = st.number_input("Max heatmaps (0 = no limit)", min_value=0, value=0, step=1, key="ocr_max_heatmaps")
        selected_ocr_params = st.multiselect(
            "Limit OCR to selected parameters (leave empty for all)",
            ALL_PARAM_DISPLAY_OPTIONS,
            default=[],
            key="ocr_selected_params",
        )
        st.caption(f"OCR runner: {OCR_RUNNER_NAME}")
        ...
        if run_ocr:
            ...
            with st.spinner("Running OCR..."):
                res = run_ocr_generate_csv(
                    ...
                    max_heatmaps=(None if max_heatmaps == 0 else int(max_heatmaps)),
                    debug=debug,
                    selected_parameters=(selected_ocr_params or None),
                )
            ...
            if isinstance(res, dict):
                if res.get("used_manifest"):
                    st.caption("Manifest pairing detected: Step 2 used the manifest-aware OCR flow.")
                elif OCR_RUNNER_NAME == "ocr_csv_generator_v2":
                    st.caption("No manifest found. Step 2 used the v2 fallback filename-based OCR flow.")

                if res.get("selected_parameters"):
                    chosen = ", ".join(res.get("selected_parameters", []))
                    st.caption(f"Processed parameter filter: {chosen}")
...
<div class="subtle">
Uses one selected metric folder from Step 2, plus Master/Router ESX files and either an uploaded, saved, or popup-created <code>site_geometry.json</code>.
Outputs go to <code>runs/&lt;router_name&gt;/rvr_outputs/&lt;metric&gt;</code>.
</div>
""",
    unsafe_allow_html=True,
...
if not PARAM_SCRIPT.exists():
    ...
    router_site_geom_existing = (current_router_dir / "site_geometry.json") if current_router_dir else None
    rvr_site_geom_mode = st.radio(
        "Site geometry",
        ["Upload existing JSON", "Create from floor-plan popup", "Use saved JSON"],
        index=["Upload existing JSON", "Create from floor-plan popup", "Use saved JSON"].index(default_site_geometry_mode(router_site_geom_existing)),
        horizontal=True,
        key="rvr_site_geom_mode",
    )

    site_geom = None
    if rvr_site_geom_mode == "Upload existing JSON":
        site_geom = st.file_uploader(
            "Upload site_geometry.json",
            type=["json"],
            accept_multiple_files=False,
    ...
    elif rvr_site_geom_mode == "Create from floor-plan popup":
        st.caption("On Run, desktop floor-plan windows will open so you can click the matching anchor points. DUT is auto-detected from the router ESX files when possible.")
    else:
        st.caption("The app will reuse the saved site_geometry.json for this router folder, if it exists.")
    ...
    if run_rvr:
        ...
        router_site_geom = run_dir / "site_geometry.json"
        try:
            with st.spinner("Preparing site geometry..."):
                resolve_site_geometry(
                    mode=rvr_site_geom_mode,
                    target_path=router_site_geom,
                    uploaded_file=site_geom,
                    master_esx_path=(esx_master_store / master_choice),
                    router_esx_paths=[esx_router_store / rn for rn in router_choices],
                    all_esx_paths=[esx_master_store / master_choice] + [esx_router_store / rn for rn in router_choices],
                )
        except Exception as exc:
            st.error(f"Site geometry setup failed: {exc}")
            st.stop()
...
<div class="subtle">
Upload WITH MESH CSVs, WITHOUT MESH CSVs, one or more ESX files, and either upload a shared <code>site_geometry.json</code> or create it from floor-plan popups.
Outputs go to <code>runs/&lt;router_name&gt;/compare_outputs/&lt;metric&gt;</code>.
</div>
""",
    unsafe_allow_html=True,
...
if not COMPARE_SCRIPT.exists():
    ...
    compare_site_geom_existing = (current_router_dir / "compare_inputs" / "_shared" / "site_geometry.json") if current_router_dir else None
    compare_site_geom_mode = st.radio(
        "Shared site geometry",
        ["Upload existing JSON", "Create from floor-plan popup", "Use saved JSON"],
        index=["Upload existing JSON", "Create from floor-plan popup", "Use saved JSON"].index(default_site_geometry_mode(compare_site_geom_existing)),
        horizontal=True,
        key="compare4_site_geom_mode",
    )

    compare_site_geom = None
    if compare_site_geom_mode == "Upload existing JSON":
        compare_site_geom = st.file_uploader(
            "Upload site_geometry.json",
            type=["json"],
            accept_multiple_files=False,
    ...
    elif compare_site_geom_mode == "Create from floor-plan popup":
        st.caption("On Run, desktop floor-plan windows will open so you can click the matching anchor points. DUT is auto-detected from the router ESX files when possible.")
    else:
        st.caption("The app will reuse the saved shared site_geometry.json for this router folder, if it exists.")
    ...
    if run_compare:
        ...
        try:
            with st.spinner("Preparing shared site geometry..."):
                all_compare_esx_paths = sorted(compare_esx_store.glob("*.esx"))
                resolve_site_geometry(
                    mode=compare_site_geom_mode,
                    target_path=site_geom_path,
                    uploaded_file=compare_site_geom,
                    all_esx_paths=all_compare_esx_paths,
                )
        except Exception as exc:
            st.error(f"Site geometry setup failed: {exc}")
            st.stop()
