...
#         router_paths = [p for p in esx_pool if os.path.abspath(str(p)) != os.path.abspath(str(master_esx_path))]
...
# </div>
# <div class="subtle">
# Uses one selected metric folder from Step 1, plus Master/Router ESX files and either an uploaded, saved, or popup-created <code>site_geometry.json</code>.
...
#     if question:
#         rebuild_needed = not kb_path.exists()
#         if not rebuild_needed:
#             try:
#                 kb = LocalKnowledgeBase.load(kb_path)
#                 has_new_style_chunks = any(
#                     (getattr(chunk, "source_type", "") in {"curve_ranking", "mesh_curve_comparison"})
#                     or ("|" in getattr(chunk, "title", ""))
#                     for chunk in kb.chunks[:50]
#                 )
#                 rebuild_needed = (not kb.chunks) or (not has_new_style_chunks)
#             except Exception:
#                 rebuild_needed = True
#         if rebuild_needed:
#             kb = LocalKnowledgeBase.build_from_run_folder(current_router_dir)
