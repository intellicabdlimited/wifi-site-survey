...
#     ),
#     dict(
#         key="secondary_signal_strength",
#         name="Secondary Signal Strength",
#         preset_edges=DBM_EDGES_FALLBACK,
#         axis_label="dBm (Higher is Better)",
#         expected_sign="negative",
#         fixed_style="dbm",
...
#     ),
#     dict(
#         key="tertiary_signal_strength",
#         name="Tertiary Signal Strength",
#         preset_edges=DBM_EDGES_FALLBACK,
#         axis_label="dBm (Higher is Better)",
#         expected_sign="negative",
#         fixed_style="dbm",
...
#         scale_kind="numeric",
#         has_band=True,
#         preset_minmax=(5.0, 40.0),   # SNR-only fixed legend endpoints
#     ),
#     dict(
#         key="noise",
#         name="Noise",
...
# def parse_floor_from_text(path_or_name: str) -> str | None:
#     s = norm_for_match(path_or_name)
#     # Look for `for <floor> on <band>` pattern
#     m = re.search(r"for\s+(.+?)\s+on\s+(?:the\s+)?(?:2\.4|5|6)\s*ghz", s, flags=re.IGNORECASE)
#     if m:
#         floor = m.group(1).strip()
#         return floor.title() if floor.islower() else floor
#     return None
...
#     if re.search(r"\bsecondary\s+signal\s+strength\b", t):
#         return "secondary_signal_strength"
#     if re.search(r"\btertiary\s+signal\s+strength\b", t):
...
#     if re.search(r"\bnumber\s+of\s+(?:aps|access\s+points)\b", t):
...
#     preset_minmax = metric_cfg.get("preset_minmax", None)

#     L, R, det_dbg = detect_left_right_label_blocks(bgr, det_upscale=DET_UPSCALE)
#     if L is None or R is None:
#         if preset_minmax is not None:
#             mn, mx = float(preset_minmax[0]), float(preset_minmax[1])
...
#     # SNR fallback (or any metric with preset_minmax)
#     if mn is None or mx is None:
#         if preset_minmax is not None:
#             preset_mn, preset_mx = preset_minmax
#             if mn is None:
#                 mn = float(preset_mn)
#             if mx is None:
#                 mx = float(preset_mx)
#             dlog(f"OCR fallback used for {os.path.basename(scale_path)} -> mn={mn}, mx={mx}")
#         else:
#             raise ValueError(
...
#             df["value"] = pd.to_numeric(df.get("raw_value", np.nan), errors="coerce")
#         else:
...
# def is_summary_heatmap_file(path: str) -> bool:
#     s = Path(path).name.lower()
#     return any(keyword in s for keyword in ["summary", "overview", "report", "total", "aggregate"])
...
#             continue
#         if is_summary_heatmap_file(p.name):
#             continue
#         heatmaps.append(str(p))

#     # Do not sort; keep extraction order, then later de-duplicate by key in run_ocr_generate_csv.
#     return heatmaps
...
#         # Skip files likely to be summary or non-data heatmaps
#         if is_summary_heatmap_file(hm_name):
#             dlog(f"Skipping summary-like heatmap: {hm_name}")
#             continue
...
#         router_key = device_from_filename(hm_name)
#         band = parse_band_from_text(hm_name) or ""
#         floor = parse_floor_from_text(hm_name) or ""
#         dedupe_key = (router_key, metric_key, floor.lower().strip(), band.lower().strip())
#         if dedupe_key in seen_heatmap_keys:
#             dlog(f"Skipping duplicate heatmap entry: {hm_name} (key {dedupe_key})")
#             continue
#         seen_heatmap_keys.add(dedupe_key)
...
def _normalize_numeric_candidate(v: float, expected_sign: str = "any", range_hint=None):
    v = float(v)
    if expected_sign == "negative":
        v = -abs(v)
    elif expected_sign == "positive":
...
def _ocr_numeric_observations(img: np.ndarray, cfg: str):
    observations = []

    data = _tess_data(img, cfg)
    ...
    for txt, conf in zip(texts, confs):
        ...
        if conf_val < 0:
            conf_val = 0.0
        for v in vals:
            observations.append({"value": float(v), "conf": conf_val, "source": "data", "text": txt})

    txt = _tess_string(img, cfg).strip()
    if txt:
        vals = _parse_signed_numbers(txt)
        for v in vals:
            observations.append({"value": float(v), "conf": 0.0, "source": "string", "text": txt})
...
def ocr_number_from_crop(num_crop_bgr: np.ndarray, expected_sign: str = "any", range_hint=None):
    ...
    variants = [("thr", thr)]
    inv = 255 - thr
    if inv is not None and inv.size == thr.size:
    ...
    for variant_name, variant_img in variants:
        for cfg_name, cfg in cfgs:
            obs = _ocr_numeric_observations(variant_img, cfg)
            for item in obs:
                raw_vals.append(float(item["value"]))
                raw_texts.append(f"{variant_name}/{cfg_name}:{item['text']}")
                norm = _normalize_numeric_candidate(item["value"], expected_sign=expected_sign, range_hint=range_hint)
                if norm is None:
                    continue
                ...
                rec["count"] += 1
                rec["sum_conf"] += float(item.get("conf", 0.0) or 0.0)
                rec["max_conf"] = max(rec["max_conf"], float(item.get("conf", 0.0) or 0.0))
                rec["sources"].add(f"{variant_name}/{cfg_name}/{item.get('source', 'unknown')}")
                if len(rec["texts"]) < 6:
                    rec["texts"].append(str(item.get("text", "")))

    if not bucket:
        return None, {"txt": " | ".join(raw_texts), "raw": raw_vals}

    scored = []
    for value, rec in bucket.items():
        scored.append(
            {
                "value": float(value),
                "count": int(rec["count"]),
                "sum_conf": float(rec["sum_conf"]),
                "max_conf": float(rec["max_conf"]),
                "digit_len": int(_candidate_digit_len(value)),
    ...
    return best, {
        "txt": " | ".join(raw_texts),
        "raw": raw_vals,
        "cands": [float(r["value"]) for r in scored],
        "scores": scored,
        "count": int(best_row["count"]),
        "sum_conf": float(best_row["sum_conf"]),
        "max_conf": float(best_row["max_conf"]),
    }
...
def ocr_minmax_from_scale_path(scale_path: str, metric_cfg: dict, debug_dir: str | None = None, tag: str = ""):
    ...
    def pick_best(a, a_meta, b, b_meta):
        ...
        def meta_score(value, meta):
            return (
                int((meta or {}).get("count", 0)),
                float((meta or {}).get("sum_conf", 0.0)),
                float((meta or {}).get("max_conf", 0.0)),
                int(_candidate_digit_len(value)),
            )

        a_score = meta_score(a, a_meta)
        b_score = meta_score(b, b_meta)
        if b_score > a_score:
            return b, {"picked": "full", "num": a_meta, "full": b_meta, "num_score": a_score, "full_score": b_score}
        return a, {"picked": "num", "num": a_meta, "full": b_meta, "num_score": a_score, "full_score": b_score}
